...
var hoccer_client = {
  ...
  share_callbacks : {
    bring_share_interface_to_front : function() {
      $("#hoccer_share").attr("src", "/legacy/images/tab_share-active.png");
      $("#hoccer_receive").attr("src", "/legacy/images/tab_receive-inactive.png");
      $("#receive_box").hide();
      $("#share_box").show();
    },
  ...
  receive_callbacks : {
    bring_receive_interface_to_front : function() {
      $("#hoccer_receive").attr("src", "/legacy/images/tab_receive-active.png");
      $("#hoccer_share").attr("src", "/legacy/images/tab_share-inactive.png");
      $("#share_box").hide();
      $("#receive_box").show();
    },
    ...
    if (options.content_type.match(/^image/)) {
      ...
      Shadowbox.open({
        content:    "<table><tr><td id='vc_image'><a href='"+ options.upload_url + "'>" +
                    "<img src='/legacy/images/vcard-icon.png' /></a></td>" +
      ...
      if (shadow_url = options.data.match(/^https?:\/\/.+$/)) {
        Shadowbox.open({
          content:    shadow_url[0],
          title:      shadow_url[0],
          player:     "iframe"
        });
      } else {
        Shadowbox.open({
          content: "<div class='received_text'>" + options.data,
          player:  "html"
        });
      }
    }
